...
public class CrystalPathRoom extends SpecialRoom {
	...
	public void paint(Level level) {
		...
		level.drop(Generator.randomItem(), level.pointToCell(rooms[shuffle == 1 ? 2 : 3].center()));
		level.drop(Generator.randomItem(), level.pointToCell(rooms[shuffle == 1 ? 3 : 2].center()));

		level.drop(Generator.randomItem(), level.pointToCell(rooms[shuffle == 1 ? 0 : 1].center()));
		level.drop(Generator.randomItem(), level.pointToCell(rooms[shuffle == 1 ? 1 : 0].center()));

		//player can only see these if they unlock the previous doors, so don't count them for exploration
		level.drop(Generator.randomItem(), shuffle == 1 ? prize1 : prize2).autoExplored = true;
		level.drop(Generator.randomItem(), shuffle == 1 ? prize2 : prize1).autoExplored = true;
